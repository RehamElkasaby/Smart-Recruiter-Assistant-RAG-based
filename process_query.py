...
def process_query(query: str, vector_store=None, llm=None):
    ...
    if is_summarize(query):
        return answer_summarize(query, vector_store, llm, candidate_names)  # Updated
...
def is_summarize(query: str) -> bool: 
    return query.lower().startswith(("What Skills", "summerize"))
...
# Use full CVs for summarization
